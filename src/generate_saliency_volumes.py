...
class GaussianBlur2D(nn.Module):
    def __init__(self):
        ...
        kernel = torch.cuda.tensor(kernel, dtype=torch.float16).unsqueeze(0).unsqueeze(0)
        self.weight = nn.Parameter(data=kernel, requires_grad=False)
...
saliency_volumes = torch.from_numpy(get_saliency_volumes(filenames, progress_bar=True))

print("Generating saliency volumes...")
temporal_maps = torch.cuda.zeros([len(filenames),25,H,W], dtype=torch.float16)

conv1D = GaussianBlur1D()
conv2D = GaussianBlur2D()
...
for i, saliency_volume in enumerate(tqdm(saliency_volumes)):
    ...
    for x in range(W):
        for y in range(H):
            temporal_maps[i,:,y,x] = conv2D.forward(temporal_maps[i,:,y,x])
