...
for i, saliency_volume in enumerate(tqdm(saliency_volumes)):
    ...
    for ts in np.unique([ts for ts, _ in fix_timestamps]):
        print(np.array(ndimage.filters.gaussian_filter(temporal_maps[i,ts-1], 25)))
        temporal_maps[i,ts-1] = np.array(ndimage.filters.gaussian_filter(temporal_maps[i,ts-1], 25))
