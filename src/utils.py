...
H = 480
TIMESPAN = 5000
MAX_PIXEL_DISTANCE = cp.linalg.norm([W, H])
...
def get_saliency_volumes(filenames,
    ...
    for filename in filenames:
        ...
        for i, observer in enumerate(fixations):
            ...
            for fixation in observer:
                distances = distance.cdist([fixation], locations[i], 'euclidean')[0][..., cp.newaxis]
                time_diffs = abs(timestamps[i] - est_timestamp)
                min_idx = cp.argmin(etw * time_diffs + distances)
    ...
    return saliency_volumes, cp.mean(errors) / MAX_PIXEL_DISTANCE

def get_heat_image(image):
    return cv2.cvtColor(cv2.applyColorMap(cp.uint8(255 * image), cv2.COLORMAP_HOT), cv2.COLOR_BGR2RGB)

RATIO = 0.9
def format_image(heatmap, image, max_value):
    extended_map = heatmap / max_value
    hsv = cv2.cvtColor(image, cv2.COLOR_RGB2HSV)
    factors = cp.clip(2 * extended_map, 0, 1)
    hsv[:,:,1] = cp.uint8(factors * hsv[:,:,1])
    hsv[:,:,2] = cp.uint8((RATIO * factors + (1 - RATIO)) * hsv[:,:,2])

    return get_heat_image(extended_map[:,:,cp.newaxis]), cv2.cvtColor(hsv, cv2.COLOR_HSV2RGB)

def animate(maps, image, normalized=False):
    fig = plt.figure(figsize=(13, 6))
    max_value = cp.max(maps);
    formatted_images = []

    for map in maps:
        heatmap, image_heatmap = format_image(map, image, cp.max(map) if normalized else max_value)
        im = plt.imshow(cp.concatenate((heatmap, image_heatmap), 1), animated=True)
